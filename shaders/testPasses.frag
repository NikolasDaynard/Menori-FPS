...
pass {
invertPass
vec4 effect(vec4 color, Image tex, vec2 texture_coords, vec2 screen_coords, colorPass)
{
    vec2 screenCord = vec2((((vertexPosition.xy / vertexPosition.w) * 0.5) + 0.5));

    vec4 colorPassPixel = Texel(colorPass, screenCord)

    return vec4((1 - colorPassPixel).rgb, 1.0);
}
}
